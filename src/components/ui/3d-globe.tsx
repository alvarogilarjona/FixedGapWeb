  ...
  return (
    <group ref={groupRef} visible={isVisible}>
      ...
      <group ref={imageGroupRef} position={topPosition}>
        <Html center sprite style={{ pointerEvents: isVisible ? "auto" : "none", opacity: isVisible ? 1 : 0, transition: "opacity 0.15s ease-out" }}>
          <div className="relative flex flex-col items-center" onMouseEnter={handlePointerEnter} onMouseLeave={handlePointerLeave} onClick={handleClick}>
            {hovered && marker.info && (
              <div className="absolute bottom-full mb-1 whitespace-nowrap rounded bg-slate-900/95 px-2 py-1 shadow-lg pointer-events-none">
                <p className="text-[10px] font-semibold text-white leading-tight">{marker.label}</p>
                <p className="text-[9px] text-slate-300 leading-tight mt-0.5">{marker.info}</p>
              </div>
            )}
            <div className="relative">
              {!hovered && <span className="absolute inset-0 rounded-full bg-slate-400/20" style={{ animation: "pulse-ring 2.5s ease-out infinite" }} />}
              <div className={cn("relative cursor-pointer overflow-hidden rounded-full bg-neutral-900 shadow-md transition-transform duration-150", hovered && "scale-110 ring-2 ring-white/60")} style={{ width: "20px", height: "20px" }}>
